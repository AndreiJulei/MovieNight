...
void main() {
  ...
  float energyScore = max(innerRing + innerEye, outerEyeGlow + outerBgGlow);
  vec3 eyeEnergy = uEyeColor * uIntensity * clamp(energyScore - pupil, 0.0, 3.0);

  // Transparent Background Calculation (Only the flaming eye is rendered)
  float alpha = clamp(energyScore * 1.4, 0.0, 1.0);
  alpha = smoothstep(0.02, 0.95, alpha);

  gl_FragColor = vec4(eyeEnergy, alpha);
}
`;
...
export default function EvilEye({
  ...
  useEffect(() => {
    ...
    // Window-level mouse tracking so Sauron follows cursor anywhere on screen
    function onMouseMove(e: MouseEvent) {
      if (!container) return;
      const rect = container.getBoundingClientRect();
      const centerX = rect.left + rect.width / 2;
      const centerY = rect.top + rect.height / 2;
      mouse.tx = Math.max(-1.5, Math.min(1.5, (e.clientX - centerX) / 180));
      mouse.ty = Math.max(-1.5, Math.min(1.5, -(e.clientY - centerY) / 180));
    }
  ...
  return (
    <div
      ref={containerRef}
      className={`relative h-full w-full bg-transparent overflow-hidden ${className}`}
    />
  );
}
